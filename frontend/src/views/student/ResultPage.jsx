...
import { Code, Visibility, VisibilityOff, Search, CheckCircle, WorkspacePremium } from '@mui/icons-material';
...
// Minimum score for a certificate — mirrors backend CERT_PASS_THRESHOLD.
const CERT_PASS_THRESHOLD = 50;
...
const ResultPage = () => {
  ...
  const handleGetCertificate = async (result) => {
    try {
      setCertLoadingId(result._id);
      const { data } = await axiosInstance.post(
        `/api/certificates/issue/${result._id}`,
        {},
      ...
      toast.error(err.response?.data?.message || 'Certificate not available for this result');
    } finally {
                  ...
                  <TableBody>
                    {results.map((result) => (
                      <TableRow key={result._id}>
                        ...
                        <TableCell>
                          {result.percentage >= CERT_PASS_THRESHOLD ? (
                            <Button
                              size="small"
                              variant="contained"
                              startIcon={<WorkspacePremium />}
                              disabled={certLoadingId === result._id}
                              onClick={() => handleGetCertificate(result)}
                            >
                              {certLoadingId === result._id ? '...' : 'Certificate'}
                            </Button>
                          ) : (
                            <Chip size="small" label="Not eligible" variant="outlined" />
                          )}
                        </TableCell>
                        <TableCell>
          ...
          <DialogContent dividers>
            {activeCert && (
              <>
                ...
                <ShareCertificate certificate={activeCert} />
              </>
            )}
          </DialogContent>
          <DialogActions>
