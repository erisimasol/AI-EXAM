...
export default function CheatingTable() {
  ...
  const handleExportCsv = async () => {
    try {
      const res = await axiosInstance.get('/api/users/cheatingLogs/export/csv', {
      ...
      link.href = url;
      link.setAttribute('download', `incident-report-${selectedExamId}.csv`);
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      console.error('Failed to export incident report', err);
    }
  };
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Copy/Paste Attempts</TableCell>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((log, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.copyPasteAttemptCount)}
                        label={log.copyPasteAttemptCount || 0}
                        color={getViolationColor(log.copyPasteAttemptCount || 0)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        label={log.severity || 'none'}
                        color={
                          log.severity === 'high'
                            ? 'error'
                            : log.severity === 'medium'
